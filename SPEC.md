...
| Video speed controller | [Video Speed Controller](https://addons.mozilla.org/en-US/firefox/addon/videospeed/) | Overlay + page-level shortcuts on any HTML5 video. `S` / `D` slower / faster, `Z` / `X` seek 10s, `R` toggle preferred speed, `V` hide overlay; speed fightback against sites that reset the rate |
